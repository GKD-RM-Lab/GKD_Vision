...
yolo_kpt::yolo_kpt() {
  ...
  std::shared_ptr<ov::Model> model =
      core.read_model(params.model_path_xml, params.model_path_bin);
  ...
  ov::AnyMap cpu_config = {
      ov::hint::performance_mode(
          ov::hint::PerformanceMode::LATENCY), // 延迟优先（单帧推理）
...
cv::Mat yolo_kpt::letter_box(cv::Mat &src, int h, int w,
...
cv::Rect yolo_kpt::scale_box(cv::Rect box, std::vector<float> &padd,
...
yolo_kpt::scale_box_kpt(std::vector<cv::Point2f> points,
                        std::vector<float> &padd, float raw_w, float raw_h,
                        int idx) {
...
void yolo_kpt::generate_proposals(
    int stride, const float *feat,
    std::vector<Object> &objects) { // 后处理部分，重做检测层
  ...
  for (int anchor = 0; anchor < ANCHOR; anchor++) { // 对每个anchor进行便利
    for (int i = 0; i < feat_h; i++) {              // self.grid[i][..., 0:1]
      for (int j = 0; j < feat_w; j++) {            // self.grid[i][..., 1:2]
        ...
        float cof = std::min(box_prob * ma

        // tp2 = getTime();x_prob, 1.0);
        ...
        obj.prob = cof;
        if (KPT_NUM != 0) {
          for (std::vector<cv::Point_<float>>::size_type k = 0; k < KPT_NUM;
               k++) {
...
std::vector<yolo_kpt::Object> yolo_kpt::work(cv::Mat src_img) {
  ...
  //    cv::dnn::softNMSBoxes(boxes, confidences, picked_useless,
  //    CONF_THRESHOLD, NMS_THRESHOLD, picked);
...
int yolo_kpt::findMissingCorner(
    const std::vector<cv::Point2f> &trianglePoints) {
...
std::string yolo_kpt::label2string(int num) {
  std::vector<std::string> class_names = {"B1", "B2", "B3", "B4", "B5",
                                          "BO", "BS", "R1", "R2", "R3",
                                          "R4", "R5", "RO", "RS"};
...
// 可视化results
cv::Mat yolo_kpt::visual_label(cv::Mat inputImage,
  ...
  if (result.size() > 0) {
    for (size_t j = 0; j < result.size(); j++) {
      ...
      for (size_t i = 0; i < result[j].kpt.size(); i++) {
        ...
        cv::putText(inputImage, text,
                    cv::Point(result[j].kpt[i].x, result[j].kpt[i].y),
      ...
      if (result[j].kpt.size() == 4) {
        cv::line(inputImage, result[j].kpt[0], result[j].kpt[1],
                 cv::Scalar(0, 255, 0), 5);
        cv::line(inputImage, result[j].kpt[1], result[j].kpt[2],
                 cv::Scalar(0, 255, 0), 5);
        cv::line(inputImage, result[j].kpt[2], result[j].kpt[3],
                 cv::Scalar(0, 255, 0), 5);
        cv::line(inputImage, result[j].kpt[3], result[j].kpt[0],
                 cv::Scalar(0, 255, 0), 5);
        char text[50];
        std::sprintf(text, "%s - P%.2f", label2string(result[j].label).c_str(),
                     result[j].prob);
        cv::putText(inputImage, text,
                    cv::Point(result[j].kpt[3].x, result[j].kpt[3].y),
                    cv::FONT_HERSHEY_SIMPLEX, 1.2, cv::Scalar(0, 0, 255), 3);
        // pnp结果
        if (result[j].pnp_is_calculated == 1) {
          char text[50];
          std::cout << result[j].pnp_tvec << std::endl;
          std::sprintf(text, "x%.2fy%.2fz%.2f",
                       result[j].pnp_tvec.at<double>(0),
                       result[j].pnp_tvec.at<double>(1),
                       result[j].pnp_tvec.at<double>(2));
          cv::putText(
              inputImage, text,
              cv::Point(result[j].kpt[3].x + 10, result[j].kpt[3].y + 30),
      ...
      if (result[j].kpt.size() == 3) {
        ...
        cv::line(inputImage, result[j].kpt[1], result[j].kpt[2],
                 cv::Scalar(0, 255, 0), 5);
        cv::line(inputImage, result[j].kpt[2], result[j].kpt[0],
                 cv::Scalar(0, 255, 0), 5);
        char text[50];
        std::sprintf(text, "%s - %d", label2string(result[j].label).c_str(),
                     result[j].kpt_lost_index);
        cv::putText(inputImage, text,
                    cv::Point(result[j].kpt[2].x, result[j].kpt[2].y),
                    cv::FONT_HERSHEY_SIMPLEX, 1.2, cv::Scalar(0, 0, 255), 3);
        // pnp结果
        if (result[j].pnp_is_calculated == 1) {
          char text[50];
          ...
          cv::putText(
              inputImage, text,
              cv::Point(result[j].kpt[2].x + 10, result[j].kpt[2].y + 30),
              cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 255), 3);
        }
      }
    }
...
/*剔除不在判定框中的特征点*/
void yolo_kpt::removePointsOutOfRect(std::vector<cv::Point2f> &kpt,
                                     const cv::Rect2f &rect) {
  // 使用 remove_if + erase 在原地剔除不在矩形内的点
  kpt.erase(std::remove_if(kpt.begin(), kpt.end(),
